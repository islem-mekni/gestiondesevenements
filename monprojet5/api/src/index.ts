...
fastify.post("/joueurs", (request, reply) => {
  const { nom, classe } = request.body as { nom: string, classe: string }
...
fastify.post("/scores", (request, reply) => {
  const { joueur_id, question_id, points } = request.body as { joueur_id: number, question_id: number, points: number }
